...
- [ ] **A DAT name change makes a second copy of a game, because every lookup that would find the first one is keyed on the name.** Found 2026-08-05, while fixing the XML-entity bug that left 871 Redump names carrying a literal `&amp;`. Half the machinery for this already exists: `dat_import.rs:272-278` deliberately *preserves an existing media ID* on re-import "without orphaning collection rows or media assets that reference that ID". But it only fires when the existing row is found, and both finders are name-keyed — `find_media_by_release_and_rom_name` (release id, which embeds the title slug, plus the ROM filename) and `find_media_by_dat_name` (the new name). Those are the only two media lookups in `retro-junk-db/src/operations.rs:438,456`; there is no content-keyed lookup at all. Hashes are consulted only *after* a row is found, to decide unchanged-vs-updated (`dat_import.rs:259`). So a corrected title is unfindable, and `make_work_id`/`make_release_id`/`make_media_id` (`dat_import.rs:394-424`, all slug-derived) mint a fresh triple beside the old one with identical hashes.

  Two consequences. Content-based re-binding then sees two candidates for one disc and correctly refuses to guess, so `archive_releases.binding_state` drops to `unresolved` and the release reads unidentified — which is how this was noticed. And nothing removes the orphan. Cleaned up by hand this time (871 media, 854 releases, 551 works deleted after checking every foreign key; backup at `catalog.db.pre-dupe-cleanup-20260805`).

  The fix is a third lookup, not a re-keying: find media by platform + content as the last fallback, and reuse that row's ID. **Measured 2026-08-05, and the choice of content key matters.** Keying on the media row's own `crc32`/`sha1`/`file_size` is *not* safe: 1043 rows share such a key with something else, 1029 of them inside a single platform across 413 groups — and every one is on a disc platform (saturn 186, ps1 137, pcecd 47, segacd 35, dreamcast 8; zero on cartridges). They are legitimately distinct entries: `Monster Lair (USA)` against `Wonder Boy III - Monster Lair (Japan) (Rev 3)`, `Kaze no Densetsu Xanadu (Japan) (Demo)` against the full game, and a family of eight Dreamcast cheat discs (GameShark CDX, Action Replay, Planet DC) sharing one 5,729,472-byte track. The cause is that a disc media row's hashes are the *largest data track* only (`dat_import.rs:226`), and CD releases routinely share a data track while differing in audio tracks. Keying on the **complete track set** — every track's sha1 and size in track order, falling back to the primary hash where there are no tracks — collapses all 413 groups to zero, and is unique across all 94,481 media rows in the current catalog. That is the same "complete track set" notion disc verification already relies on. The wrinkle is that a title change also mints a new work and release, so the recovered media has to be re-pointed at the new release (keeping collection and asset references intact) and the emptied work/release pruned — which is why this is not a one-liner and wants review rather than a quiet patch.
