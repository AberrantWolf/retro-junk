...
- [ ] **The catalog gains a second copy of a game when a DAT name changes, and never retires the first.** Found 2026-08-05, while fixing the XML-entity bug that made 871 Redump names carry a literal `&amp;`. Re-importing the corrected DATs did not update those rows: the name is part of the slug, so a corrected name is a *new* work/release/media triple and the old one stays behind with identical hashes. Two consequences. Content-based re-binding then sees two candidates for one disc and correctly refuses to guess, so `archive_releases.binding_state` drops to `unresolved` and the release reads unidentified — which is exactly how this was noticed. And nothing ever removes the orphan. Cleaned up by hand this time (871 media, 854 releases, 551 works deleted after checking every foreign key; backup at `catalog.db.pre-dupe-cleanup-20260805`). Wants the importer to recognise "same platform, same hashes, different name" as a rename of an existing entry rather than a new one — or a supported `catalog prune-orphans` that removes catalog rows no DAT import produces any more.
