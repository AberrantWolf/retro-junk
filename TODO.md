...
- [x] **A DAT name change makes a second copy of a game, because every lookup that would find the first one is keyed on the name.** Found 2026-08-05, while fixing the XML-entity bug that left 871 Redump names carrying a literal `&amp;`, and closed the same week by taking the title out of the key entirely rather than adding a third lookup beside the two name-keyed ones.

  Catalog ids were `{platform}:{slug(title)}` and derivations of it, so the title was part of the primary key of `works`, `releases` and `media` — and of the ids written into every archive manifest. A medium's id is now folded from its complete ordered track set (`retro-junk-catalog/src/content_id.rs`); works and releases are minted once and found by their natural key; the title is a label a re-import simply updates. Manifests carry no catalog ids at all, so a carrier resolves from the digests it recorded, on whatever machine is asking.

  **The choice of content key was measured 2026-08-05 and matters.** Keying on the media row's own `crc32`/`sha1`/`file_size` is *not* safe: 1043 rows share such a key with something else, 1029 of them inside a single platform across 413 groups — every one on a disc platform (saturn 186, ps1 137, pcecd 47, segacd 35, dreamcast 8; zero on cartridges). They are legitimately distinct entries: `Monster Lair (USA)` against `Wonder Boy III - Monster Lair (Japan) (Rev 3)`, `Kaze no Densetsu Xanadu (Japan) (Demo)` against the full game, and a family of eight Dreamcast cheat discs sharing one 5,729,472-byte track. The cause is that a disc media row's hashes are the *largest data track* only, and CD releases routinely share a data track while differing in audio. The **complete track set** collapses all 413 groups to zero and is unique across all 94,481 media rows. That is the key.

  The 871 orphans from the incident were cleaned up by hand before the fix (871 media, 854 releases, 551 works deleted after checking every foreign key; backup at `catalog.db.pre-dupe-cleanup-20260805`). See `IDENTIFICATION.md`.

- [x] **A gamelist entry could never be corrected once written.** Done (2026-08-06). `upsert_game_metadata` returned without writing whenever the entry's `<path>` was already present — it inserted, it never updated — so a release whose catalog title was corrected, or whose artwork arrived after its first sync, kept the name and the empty art tags it was created with forever. `esde::upsert_games` now merges per tag: the tags this crate renders are replaced, and everything else in the block (`<favorite>`, `<playcount>`, `<lastplayed>`, a hand-typed description) is preserved exactly as found, with a tag the new render does not produce left alone rather than blanked. It also takes many games at once, which is what makes one write per folder possible.

- [x] **A gamelist was written one game at a time, and artwork went somewhere else.** Done (2026-08-06). `SyncGamelist` targeted a release while `gamelist.xml` is one file per system folder, so the reference collection derived 305 actions that rewrote 19 files — the `n3ds` list, 111 entries, about a hundred times per run. The action is now `WorkTarget::Console`, derived from the folders playable files are actually in (`representations.relative_path`), and `sync_esde_gamelist_for_console` reads and writes each file once; the per-release form is that function with one release in it. Separately, the two halves of a projection disagreed about which folder a release publishes into — the gamelist asked the filesystem, asset projection asked the naming rule — so a `world`-region NES release built into `famicom` had its entry written to `famicom/` and its artwork copied into `nes/`. Both now ask `playable_location::release_publish_directory`.

- [x] **Every convergence action paid for its own connections, lock, and scan.** Done (2026-08-06). `execute_actions` takes [1..N] actions and acquires the database connection, the heartbeat connection, and the whole-archive lock once for the batch; only the claim stays per item, so another process can still pick up what this batch has not reached. `execute_action` is the same function with one item in it, so the release-the-claim-on-every-path guarantee cannot drift between a batch path and a single path. Holding the lock across the batch also closed a hole the shared scan opened: dropping it between items let another process change the archive while the cached walk still described the old tree. Verified safe because the one dispatch path that would re-acquire (publishing scraped media) already carries `acquire_lock: false` on the executor path. The GUI gained "Converge Selected" for a multi-row selection, via `Scope::Releases`.

- [ ] **Nothing removes gamelist entries the archive never wrote.** Found 2026-08-05. 11 entries in `roms/psx/gamelist.xml` point at files that do not exist and appear in no build evidence — leftovers from the pre-archive scrape/organize flow, under names like `Castlevania - Symphony of the Night (USA) (Track 1).chd` and `conveni-2-the-zenkoku-chain-tenkai-da-japan.chd`. Retirement can only remove names the archive knows it published, so these survive every projection. A prune restricted to entries whose `<path>` resolves to no file is safe by construction — the entry references nothing — but it deletes rows a user may have hand-authored for ROMs managed outside the archive, so it needs an explicit opt-in flag and a dry run, not a silent step inside `sync`.
...
- [x] **`ProjectAssets` and `SyncGamelist` derive as pending forever, and each one dispatches a full archive scan.** Done (2026-08-06). Three separate causes. The scan is now walked once per run and shared (`ExecContext::archive`), with the release lookup built alongside it — finding a release used to be a search through every release, once per release. Currency is recorded: a projection stores a fingerprint of the archive facts it was made from (`retro-junk-db/src/projection_state.rs`), derivation proposes it only when that no longer matches, and `summarize_convergence` counts the rest as done, so `status` on a converged library reports zero rather than six hundred. Because a source-side fingerprint cannot see a media file deleted outside retro-junk, `sync --force-projections` forgets the records for a scope; `archive project-assets` still projects unconditionally. And `SyncGamelist` moved to the folder it actually writes — see below.
...
- [ ] **Executor claim protocol: a quiet dispatch loses its claim, and nobody notices the theft.** Found in the 2026-08-03 review. The claim heartbeat piggybacks on progress callbacks (`retro-junk-work/src/executor.rs:247-259`), so any dispatch phase quiet for over `CLAIM_TIMEOUT_MINUTES` (2 min) — a long external tool run, a stalled network read — lets another process legitimately steal the claim; `refresh_claim` updates 0 rows and returns `Ok`, so the original owner keeps working (duplicated work; the archive lock prevents actual concurrent mutation). Owner-scoped `release_claim` landed 2026-08-03, so the walking dead can no longer delete the thief's live claim on the way out. Remaining: `refresh_claim` should report whether the claim is still yours (and dispatch should check it at phase boundaries), and/or a timer-based beat for quiet phases.
  - Related fragility, same file: cancellation used to be classified by comparing the error's display text to the literal `"operation cancelled"`, so any rewording silently reclassified cancellations as failures. Fixed 2026-08-06: `WorkError::Cancelled` is its own variant, `ArchiveOpsError::Cancelled` maps to it, and the three sites that returned the literal string return the variant.
...
- [x] **The default gamelist location depends on which surface wrote it.** Done: every surface now resolves roots through `retro_junk_backend::profiles::frontend_roots`, which reads the user's `metadata_dir` setting and applies overrides on top, so there is one answer per installation. Confirmed 2026-08-06. Leftover on the reference volume: `roms-metadata/` still holds 16 gamelists from before the fix, last written 2026-08-04 and no longer maintained; the live ones are inline at `roms/<console>/gamelist.xml`. Safe to delete by hand.
...
- [ ] **The vocabulary does not match the concepts.** Recorded 2026-08-05, alongside the content-id work that made it obvious. The catalog calls a physical disc or cartridge a `media`, while the archive calls the same thing a `carrier` and reserves `media` for something else again — so `catalog_media_id` on a `carriers` row is a carrier's media, and reading either crate after the other takes a beat of translation. `work`, `release` and `representation` are used consistently but say nothing about themselves: nothing in the name of a `representation` suggests "one stored form of one dump". A rename pass — `media` → `carrier` where that is what is meant — plus a written glossary would remove a standing source of confusion. It touches nearly every crate, so it wants to be its own change rather than a rider on someone else's.
