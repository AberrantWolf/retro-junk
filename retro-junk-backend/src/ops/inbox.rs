...
//!
//! Applying goes through [`crate::apply_suggestion_choice`], dismissing
//! through `retro_junk_db::work::resolve_suggestions`, and ignoring through
//! [`crate::ignore_playables`] — the same calls
//! `retro-junk suggestions apply|dismiss|ignore` make, so a decision taken
//! in any frontend and one taken at the terminal are the same decision.
//!
//! Everything a row needs to *render* is computed once, here, off any render
...
use retro_junk_db::work::{IncomingPackage, ReopenOutcome, Suggestion};

use super::OpCtx;
use crate::executor::ExecContext;
use crate::incoming::{IMPORT_SUGGESTION_KIND, ImportSuggestionPayload};
use crate::policy::AutomationPolicy;
use crate::suggestions::{OfferedActions, ScrapeSuggestionPayload, SuggestionFilter};
...
pub struct InboxItem {
    ...
    /// [`crate::suggestions`] — so a row never offers a button that does
    /// nothing.
...
impl InboxContents {
    /// What a sidebar badge counts: everything awaiting a decision.
    #[must_use]
...
/// Load everything the inbox shows, in one database pass.
pub fn load_inbox(
    db_path: &Path,
    playable_root: Option<PathBuf>,
    collection_root: Option<&Path>,
) -> Result<InboxContents, String> {
    ...
    let items = retro_junk_db::work::list_open_suggestions(&connection, None)
        ...
        .collect();
    let failed_packages = retro_junk_db::work::list_incoming_packages(&connection, Some("error"))
        .map_err(|error| error.to_string())?;
    ...
    let counts_by_kind = retro_junk_db::work::open_suggestion_counts(&connection)
        .map_err(|error| error.to_string())?;
    // A collection with no rules is the normal case, and an unreadable rule
    // store must not cost the whole inbox.
...
fn describe(suggestion: Suggestion, playable_root: Option<&Path>) -> InboxItem {
    let actions = crate::offered_actions(&suggestion);
    let (headline, location, status, details, group) = match suggestion.kind.as_str() {
        IMPORT_SUGGESTION_KIND => describe_import(&suggestion),
        crate::SCRAPE_SUGGESTION_KIND => describe_scrape(&suggestion),
        crate::ADOPT_SUGGESTION_KIND => describe_adoption(&suggestion),
        _ => (
...
#[must_use]
pub fn target_path(suggestion: &Suggestion, playable_root: Option<&Path>) -> Option<PathBuf> {
    if suggestion.target_kind != "path" {
        return None;
    }
    if suggestion.kind == crate::ADOPT_SUGGESTION_KIND {
        return playable_root.map(|root| root.join(&suggestion.target_id));
    }
    Some(PathBuf::from(&suggestion.target_id))
}
...
fn describe_adoption(suggestion: &Suggestion) -> Described {
    ...
    let platform = crate::adoption::platform_of(&payload.relative_path);
...
/// the other twenty-four. `on_applied` fires after every attempt so a
/// frontend can refresh its view as the batch progresses.
pub fn apply_many(
    exec: &ExecContext,
    ids: &[i64],
    ctx: &OpCtx,
    mut on_applied: impl FnMut(),
    ...
    for &id in ids {
        ...
        match crate::apply_suggestion_choice(exec, id, None, ctx.cancel) {
            Ok(_) => applied += 1,
...
/// Resolve suggestions without executing them. Returns the ids that were
/// actually closed, so a frontend can offer to put exactly those back.
pub fn dismiss(db_path: &Path, ids: &[i64]) -> Result<Vec<i64>, String> {
    let mut connection =
...
pub fn reopen(db_path: &Path, ids: &[i64]) -> Result<ReopenOutcome, String> {
    let mut connection =
        retro_junk_db::open_database(db_path).map_err(|error| error.to_string())?;
    retro_junk_db::work::reopen_suggestions(&mut connection, ids).map_err(|error| error.to_string())
}
...
pub fn forget_package(db_path: &Path, path: &str) -> Result<(), String> {
    let mut connection =
        retro_junk_db::open_database(db_path).map_err(|error| error.to_string())?;
...
/// under current policy — the same `process_pending_incoming` the daemon runs.
pub fn retry_package(exec: &ExecContext, path: &str, ctx: &OpCtx) -> Result<String, String> {
    let policy = AutomationPolicy::load();
    let mut connection =
        retro_junk_db::open_database(&exec.db_path).map_err(|error| error.to_string())?;
    retro_junk_db::work::requeue_incoming_package(&mut connection, path)
        .map_err(|error| error.to_string())?;
    crate::incoming::process_pending_incoming(exec, &policy, ctx.cancel)
        .map_err(|error| error.to_string())
        .map(|stats| {
            format!(
