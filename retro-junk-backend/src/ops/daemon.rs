//! Starting, stopping, and reporting on the convergence daemon from a
//! frontend.
//!
//! The daemon is a CLI subcommand by design — one install, one config, one
//! set of credentials — so a frontend launches `retro-junk daemon start`
//! rather than growing a second daemon implementation inside its own
//! process. What these commands add is a place for its output to go (the
//! CLI expects launchd, systemd, or a terminal to capture stdout; a GUI
//! launch has none of those) and a status readout built from the same PID
//! file and heartbeat `retro-junk daemon status` reads.

use std::path::PathBuf;

use crate::daemon::{daemon_log_path, process_alive, read_pid_file, remove_pid_file};

/// What a frontend's daemon panel shows.
...
/// Locate the `retro-junk` CLI: beside the current executable first (how a
...
pub fn start() -> Result<(), String> {
    ...
    // the point — quitting the frontend should not stop a running
    // convergence.
    Ok(())
}
...
/// unbounded, and a status panel only ever shows the end of it.
#[must_use]
